...
normal_attributes = ['barkbands', 'erbbands', 'melbands', 'melbands128', 'spectral_contrast_coeffs',
                     'spectral_contrast_valleys', 'beats_loudness_band_ratio', 'hpcp', 'hpcp_crest']
normal_variables = ['dmean', 'dmean2', 'dvar', 'dvar2', 'max', 'mean', 'median', 'min', 'stdev', 'var']
...
def generate_normal_attributes(variable_name):
    if number_of_commas > 0:
        ...
        print('@attribute     ' + type_of_attribute + '.' + name_of_attribute + '.' + variable_name + '    REAL')


def generate_cc_attributes(number_of_mean_commas, number_of_cov_commas, number_of_icov_commas):
    for x in range(number_of_mean_commas):
        print('@attribute     ' + type_of_attribute + '.' + name_of_attribute + '.mean_' + str(x) + '    REAL')
    for x in range(number_of_cov_commas):
        print('@attribute     ' + type_of_attribute + '.' + name_of_attribute + '.cov_' + str(x) + '    REAL')
    for x in range(number_of_icov_commas):
        print('@attribute     ' + type_of_attribute + '.' + name_of_attribute + '.icov_' + str(x) + '    REAL')
...
while repeat == 'y':
    ...
    if name_of_attribute in normal_attributes:
        ...
        number_of_commas = int(input('how many commas are there?\t'))
        for x in range(number_of_commas):
            print('@attribute     ' + type_of_attribute + '.' + name_of_attribute + '.' + '_' + str(x) + '    REAL')
        ...
        for x in range(3):
            generate_cc_attributes(number_of_mean_commas, number_of_cov_commas, number_of_icov_commas)
    else:
